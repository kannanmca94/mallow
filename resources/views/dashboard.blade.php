...
<div class="hero">
    <div class="hero-inner">
        <div>
            ...
            <div class="sub">{{ $metrics['merchant']['name'] }} &middot; {{ $metrics['cycle_start'] }} &rarr; {{ $metrics['cycle_end'] }}</div>
        </div>
        <form method="get">
            <select name="merchant" onchange="this.form.submit()">
                @foreach ($merchants as $m)
                    <option value="{{ $m->id }}" @selected($m->id === $merchant->id)>{{ $m->name }} (#{{ $m->id }})</option>
                @endforeach
            </select>
        </form>
    </div>
...
<div class="wrap">
        ...
        <div class="cards">
            <div class="card usage">
                ...
                <div class="card-body">
                    ...
                    <div class="value">{{ $topUnits ? number_format($topUnits) : '0' }} <span style="font-size:14px;font-weight:500;opacity:.7">units</span></div>
                    <div class="hint">{{ $topName ?? 'No usage aggregated' }}</div>
                </div>
            </div>
            <div class="card revenue">
                ...
                <div class="card-body">
                    ...
                    <div class="value">&#8377;{{ number_format($overage, 2) }}</div>
            ...
            <div class="card churn">
                ...
                <div class="card-body">
                    ...
                    <div class="value">{{ $churnCount }}</div>
                ...
                <table>
                    ...
                    <tbody>
                        @foreach ($top as $i => $customer)
                            <tr>
                                <td><span class="rank {{ $rankClasses[$i] ?? 'normal' }}">{{ $i + 1 }}</span></td>
                                <td>{{ $customer->name }} <span class="email">&middot; {{ $customer->email }}</span></td>
                                <td style="text-align:right;font-weight:700">{{ number_format($customer->total_units) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <section>
            ...
            @php $risks = $metrics['usage_drop_over_50_percent_month_over_month']; @endphp
            @if (empty($risks))
                ...
                <table>
                    ...
                    <tbody>
                        @foreach ($risks as $risk)
                            ...
                            @endphp
                            <tr>
                                <td>{{ $risk['customer']->name }} <span class="email">&middot; {{ $risk['customer']->email }}</span></td>
                                <td style="text-align:right">{{ number_format($old) }}</td>
                                <td style="text-align:right">{{ number_format($new) }}</td>
                                <td>
                                    <div class="drop-bar">
                                        <div class="track"><div class="fill" style="width:{{ $pct }}%"></div></div>
                                        <div class="pct">&minus;{{ $pct }}%</div>
                                    </div>
                                </td>
        ...
        <footer>
            API: <a class="muted" href="/api/merchants/{{ $merchant->id }}/dashboard">GET /api/merchants/{{ $merchant->id }}/dashboard</a> &middot;
